...
>(function ControlPanel(
  ...
  return (
    ...
    // スペクトラムを下端に置く。離れる幅には上限があり、超えたら中央に寄せる
    <div
      ref={ref}
      className={`w-fit shrink-0 ${direction === 'side' ? 'flex flex-col self-stretch' : 'pt-1'}`}
      style={layoutVars(layout)}
    >
      <div
        className={
          direction === 'side' ? 'my-auto flex h-full w-full flex-col justify-between max-h-(--panel-max)' : ''
        }
      >
        <div>
